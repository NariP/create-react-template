...
const getProjectName = async () => {
	projectName = await getUserInput(
		'Please enter your project name (or "." for current directory): ',
	);
};
...
const copyBoilerplate = ({ currentPath, projectName }) => {
	...
	const tempDir = fs.mkdtempSync(
		path.join(require("node:os").tmpdir(), "boilerplate-"),
	);
	...
	try {
		...
		if (!isEmpty) {
			...
			const answer = await getUserInput(
				"Would you like to clear the directory? (Y/N): ",
			);
			if (answer.toUpperCase() === "Y") {
				clearDirectory(currentPath);
			} else {
				throw Error(
					"Cannot proceed further as the directory is not empty (.git 제외).",
				);
			}
		}
